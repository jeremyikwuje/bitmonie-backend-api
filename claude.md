...
- **First-set-only on the customer endpoint.** `PATCH /v1/loans/:id/release-address` only accepts `NULL → value` — it rejects any change to an existing address with `ReleaseAddressAlreadySetException` (409). This closes the attack vector of a compromised customer account swapping the address right before release. If the customer typo'd their first-set address, support contacts them and ops adjusts it server-side (no customer-facing change endpoint). After release, `setReleaseAddress` rejects with the more specific `CollateralAlreadyReleasedException` (409) — the address is now bound to the actual send and is part of loan history.
